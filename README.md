...
- leaf_node_num_cells はノードにセルが何個あるかを示しているの。
  <br> => 単純にリーフノード内にも現在のセルの数を格納するエリアがある。

- Cursor の復習
  - テーブル内の場所を示すためにある。テーブルの最後の場所を記録しておくで、次のデータを挿入する場所がわかる。また、任意のページ番号、セル番号を記録しておくことで、その番号のデータにアクセスする。
